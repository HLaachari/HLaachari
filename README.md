# Hi 👋 I'm Hossam Abdelghafour Laachari

> A concise, professional GitHub profile to showcase skills, stats, and quick access to my CV.

<div align="center">
  <h3 align="center">Software Developer • Researcher • Problem Solver</h3>

  <!-- GitHub stats -->
  <p align="center">
    <img alt="GitHub stats" src="https://github-readme-stats.vercel.app/api?username=HLaachari&show_icons=true&include_all_commits=true&count_private=true&theme=dracula&hide_border=false" />
    <img alt="Top languages" src="https://github-readme-stats.vercel.app/api/top-langs/?username=HLaachari&layout=compact&langs_count=6&theme=dracula&hide_border=false" />
  </p>
</div>

---

## About me
- 🔭 I’m currently working on practical software projects and experiments that solve real-world problems.
- 🌱 I love learning new technologies and applying them in web & data-related projects.
- 💬 Ask me about JavaScript, TypeScript, React, Python, and software architecture.
- 📫 Get in touch: [laachari.i2s@uhp.ac.ma](mailto:laachari.i2s@uhp.ac.ma) • [LinkedIn](https://www.linkedin.com/in/hossam-abdelghafour-laachari-ba1891257/)

---

## Resume
You can download my resume here:
- [Hossam_Abdelghafour_LAACHARI_Resume.pdf](./Hossam_Abdelghafour_LAACHARI_Resume.pdf) (in this repository)

---

## Tech & Tools
<div align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" height="30" alt="javascript" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" height="30" alt="typescript" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" height="30" alt="react" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="30" alt="nodejs" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="30" alt="html5" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" height="30" alt="css3" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" height="30" alt="python" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" height="30" alt="csharp" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-plain.svg" height="30" alt="git" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" height="30" alt="docker" />
</div>

---

## Select highlights
- ✅ Clean, component-driven front-ends using React + TypeScript.
- ✅ API design and back-end services with Node.js and Python.
- ✅ Data cleaning, basic analysis & automation tasks.
- ✅ CI/CD with GitHub Actions & containerization.

---

## Connect & socials
<div align="left">
  <a href="https://www.linkedin.com/in/hossam-abdelghafour-laachari-ba1891257/">
    <img src="https://img.shields.io/static/v1?message=LinkedIn&logo=linkedin&color=0077B5&style=for-the-badge" height="35" alt="LinkedIn" />
  </a>
  <a href="mailto:laachari.i2s@uhp.ac.ma">
    <img src="https://img.shields.io/static/v1?message=Email&logo=gmail&color=D14836&style=for-the-badge" height="35" alt="Email" />
  </a>
  <a href="./Hossam_Abdelghafour_LAACHARI_Resume.pdf">
    <img src="https://img.shields.io/static/v1?message=Resume&logo=document&color=0A66C2&style=for-the-badge" height="35" alt="Resume" />
  </a>
</div>

---

## Currently building
- Improving a portfolio project that demonstrates end-to-end features (frontend, backend, tests, CI).
- Experimenting with small ML/data workflows (Python) to add data-driven features.

---

## Quick tips for repo visitors
- Browse /projects for ongoing work (if present).
- Open issues or PRs to start a discussion — collaboration is welcome.
- Download the resume above for a complete professional summary.

---

<br />

<div align="center">
  <img src="https://i.imgflip.com/65efzo.gif" alt="Animated" height="120" align="right" />
  <br clear="both" />
</div>

---

Thanks for visiting — if you'd like, tell me which repository you'd like summarized or what improvements you want on this README and I'll update it.

<!--
PROFILE README GENERATED: concise, professional, and responsive.
You can adapt widgets, badges, colors and the content to match your preferred tone.
-->